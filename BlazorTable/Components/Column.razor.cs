...
namespace BlazorTable
{
    ...
    public partial class Column<TableItem> : IColumn<TableItem>
    {
        ...
        /// <summary>
        ///     Aggregates table column for the footer. It can only be applied to numerical fields (e.g. int, long decimal, double,
        ///     etc.).
        /// </summary>
        [Parameter]
        ...
        public async Task SortByAsync()
        {
            if (Sortable)
            {
                if (SortColumn) SortDescending = !SortDescending;
        ...
        {
            if (Table.ItemsQueryable != null && Aggregate.HasValue && Table.ShowFooter &&
                !string.IsNullOrEmpty(Field.GetPropertyMemberInfo()?.Name))
                return Aggregate.Value switch
                {
                    AggregateType.Count => string.Format(CultureInfo.CurrentCulture, $"{{0:{Format}}}",
                        Table.ItemsQueryable.Count()),
                    AggregateType.Min => string.Format(CultureInfo.CurrentCulture, $"{{0:{Format}}}",
                        Table.ItemsQueryable.AsEnumerable().Min(c =>
                            c.GetType().GetProperty(Field.GetPropertyMemberInfo()?.Name).GetValue(c, null))),
                    AggregateType.Max => string.Format(CultureInfo.CurrentCulture, $"{{0:{Format}}}",
                        Table.ItemsQueryable.AsEnumerable().Max(c =>
                            c.GetType().GetProperty(Field.GetPropertyMemberInfo()?.Name).GetValue(c, null))),
                    _ => string.Format(CultureInfo.CurrentCulture, $"{{0:{Format}}}",
                        Table.ItemsQueryable.Aggregate(Field.GetPropertyMemberInfo()?.Name, Aggregate.Value))
                };
            return string.Empty;
        }
        ...
        protected override void OnInitialized()
        {
            ...
            if (DefaultSortDescending.HasValue) SortDescending = DefaultSortDescending.Value;

            if (DefaultSortColumn.HasValue) SortColumn = DefaultSortColumn.Value;
        }
        ...
        protected override void OnParametersSet()
        {
            ...
            if (Type == null) Type = Field?.GetPropertyMemberInfo().GetMemberUnderlyingType();
        }
    }
}