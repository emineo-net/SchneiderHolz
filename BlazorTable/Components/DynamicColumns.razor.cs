...
namespace BlazorTable
{
    ...
    public partial class DynamicColumns<TableItem>
    {
        ...
        private static Expression<Func<TModel, T>> GenerateMemberExpression<TModel, T>(PropertyInfo propertyInfo)
        {
            ...
            if (propertyInfo.ReflectedType.Name == "JObject")
            {
                var assembly = AppDomain.CurrentDomain.GetAssemblies()
                    .Where(x => x.ManifestModule.Name == "Newtonsoft.Json.dll").First();
                ...
                columnExpr = Expression.Call(
                    exttype.GetMethod("Value", new[] { typeof(IEnumerable<>).MakeGenericType(type) })
        ...
        {
            if (property.ReflectedType.Name == "JObject") return ""; // func.Invoke(data)?.ToString();

            var rawData = property.GetValue(data);
            ...
            {
                var enumType = property.GetValue(data).GetType();

                var memberInfo = enumType.GetMember(rawData.ToString());
